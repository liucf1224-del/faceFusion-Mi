...
def load_ui_layout_module(ui_layout : str) -> Any:
	print(f"DEBUG: Loading UI layout module: {ui_layout}")
	try:
		ui_layout_module = importlib.import_module('facefusion.uis.layouts.' + ui_layout)
		print(f"DEBUG: Successfully imported module: {ui_layout_module}")
		for method_name in UI_LAYOUT_METHODS:
			print(f"DEBUG: Checking method: {method_name}")
			if not hasattr(ui_layout_module, method_name):
				raise NotImplementedError
		print(f"DEBUG: All required methods found in {ui_layout_module}")
		...
		hard_exit(1)
	print(f"DEBUG: UI layout module {ui_layout} loaded successfully")
	return ui_layout_module


def get_ui_layouts_modules(ui_layouts : List[str]) -> List[ModuleType]:
	print(f"DEBUG: Getting UI layouts modules: {ui_layouts}")
	if not UI_LAYOUT_MODULES:
		for ui_layout in ui_layouts:
			print(f"DEBUG: Loading UI layout module: {ui_layout}")
			ui_layout_module = load_ui_layout_module(ui_layout)
			print(f"DEBUG: Loaded UI layout module: {ui_layout_module}")
			UI_LAYOUT_MODULES.append(ui_layout_module)
			print(f"DEBUG: Added to UI_LAYOUT_MODULES, now contains: {len(UI_LAYOUT_MODULES)} modules")
	else:
		print(f"DEBUG: UI_LAYOUT_MODULES already populated with {len(UI_LAYOUT_MODULES)} modules")
...
def init() -> None:
	...
	print(f"DEBUG: GRADIO_TEMP_DIR set to: {os.environ['GRADIO_TEMP_DIR']}")
...
def launch() -> None:
	...
	for ui_layout in state_manager.get_item('ui_layouts'):
		print(f"DEBUG: Running UI layout: {ui_layout}")
		ui_layout_module = load_ui_layout_module(ui_layout)
		ui_layout_module.run(ui)
	print("DEBUG: UI launch completed")
